/** Mirror of server/wet3StreamToken.ts for Vercel handlers. */
...
export async function obtainWet3StreamToken(
  ...
  const cookie = guestCookie ?? `wet3_user_id=${crypto.randomUUID()}`
  ...
  try {
    ...
    if (nested?.includes('/api/ad-complete')) adCompleteUrl = nested
    else if (abs.includes('/api/ad-complete')) adCompleteUrl = abs
  } catch {
    return null
  }

  if (!adCompleteUrl) return null

  const complete = await fetch(adCompleteUrl, {
    method: 'GET',
    redirect: 'manual',
    headers: { ...headers, Referer: 'https://countdownto.today/' },
  })
